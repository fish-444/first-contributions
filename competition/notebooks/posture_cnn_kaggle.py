...
#
# ## 목표가 아닌 것부터 — 0.861 은 겨냥이 아니다
#
# 0.861 은 "좌/우 횡와는 bbox 기하로 원리상 구분 불가" 라는 **폐기된
...
CEILING, MIN_FOLD, CROP, PAD = 0.861, 150, 160, 0.12
...
def mean_block(key):
    ...
    for f in RUNS[0][key]:
        ...
        out[f] = int(round(np.mean(v))) if f in ("n", "folds")             else float(np.mean(v))
    return out
