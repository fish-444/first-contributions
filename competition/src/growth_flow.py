...
# 면적 숫자는 **여기 적지 않는다** — `legal_density.TABLE` 이 정본이다.
#
# 우리 '이유자돈'(8~30kg)은 조문의 20kg 경계를 가로지른다: 조문은 초기
# 0.2(20kg 미만) · 후기 0.3(20~30kg)으로 가르는데, 자돈사 한 방에 두 무리가
# 같이 있으니 바닥을 체중으로 갈라 줄 수가 없다. 그래서 **엄한 쪽(후기 0.3)**
# 을 방 전체에 적용한다 — 조문보다 넉넉히 요구하는 쪽이고, 모자라게 잡아
# "적정"이라 답하는 것보다 안전하다.
STAGES = [
    ("포유자돈", 0, 28, 1.4, 8.0, "분만사", None),   # 젖먹이는 세지 않는다(조문 비고)
    ("이유자돈", 28, 70, 8.0, 30.0, "자돈사", ld.TABLE["새끼돼지_후기"]),
    ("육성돈", 70, 105, 30.0, 60.0, "육성사", ld.TABLE["육성돈"]),
    ("비육돈", 105, 175, 60.0, 115.0, "비육사", ld.TABLE["비육돈"]),
]
MARKET_AGE = 175
MARKET_WEIGHT = 115.0
...
# 벤치마크 — **문헌 인용이 아니라 국내 실측**이다.
# 연도별 번식통계 466행 · 농장 202개 · 2020~2023 (src/korean_farm_stats.py).
# 농가에는 "평균 대비"보다 "상위 몇 %"가 훨씬 잘 읽힌다. 덴마크만 문헌 비교값.
#
# 문헌값과 대조해 보면 우리가 쓰던 인용이 크게 틀리진 않았다:
#   한돈팜스 평균 PSY 22.8 · MSY 18.4  vs  실측 중앙 24.1 · 18.8
# 다만 **분포**를 보여줄 수 있게 됐다 — 하위 25% 는 PSY 21.9 · MSY 14.9 로
# 중앙값과 큰 격차가 있고, 개선 여지가 어디 있는지는 그쪽에서 보인다.
BENCHMARKS = {
    "국내 하위 25%": {"psy": 21.9, "msy": 14.9},
    "국내 중앙값": {"psy": 24.1, "msy": 18.8},
    "국내 상위 10%": {"psy": 27.9, "msy": 25.7},
    "덴마크(문헌)": {"psy": 31.3, "msy": 29.2},
}
