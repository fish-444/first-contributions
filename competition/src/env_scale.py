...
`barn_env_control`(지금 위험한가) · `env_anomaly`(어느 관측이 튀었는가)와
묻는 것이 다르다 — 표는 `env_anomaly` 모듈 상단에 있다. 이 모듈은 **화면에
띄울 −1~1 눈금**을 만드는 쪽이고, 산포를 IQR 로 잰다(`env_anomaly` 는 MAD).
**두 모듈의 z 를 같은 표에 나란히 놓지 말 것.**
