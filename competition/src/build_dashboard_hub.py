...
VIEWS = [
    ("farm_setup.html", "농장 등록 (처음 한 번)", "우리 농장 규모·축사동·운영 방식을 넣으면 배치 설계·필요 돈방·발정 판정 경로가 즉시 나온다", "🏠", "운영"),
    ("pc_suite.html", "★ PC 통합 콘솔 (한 파일)", "사무실에서 진행하는 화면 7개를 한 HTML 로 — 사이드바·숫자키 이동 · 모바일 제외", "🗂️", "운영"),
    ("pc_console.html", "PC 관리 콘솔 (동작)", "사무실용 — 일괄 처리·정렬·작업지시서 인쇄·단축키", "🖥️", "운영"),
    ("app_prototype.html", "앱 프로토타입 (동작)", "실제로 눌러서 돌아다니는 앱 — 탭 이동·검색·도면 드릴다운", "🐖", "운영"),
    ("app_screens.html", "앱 사용 화면", "농가가 보게 될 6개 화면 — 실데이터로 채운 하루 동선", "📱", "운영"),
    ...
    ("pigflow_console.html", "돈군흐름 관제", "분만틀에서 역산한 설계 · 필요 vs 보유 돈방 · 점유 간트 · 배치 what-if", "🔄", "운영"),
    ("farm_diagnosis.html", "실측 진단 (466농장 대비)", "순위가 아니라 거리 — 격차를 두수로, 두수를 원/년으로 · 하락 비대칭 · 계절 되돌림", "📉", "분석"),
    ("env_anomaly.html", "환경 이상치 (온·습·환기)",
     "기준 집단 안의 편차로 판정 — 사분위 밴드·날짜별 추이·알림 목록. 문턱은 필드마다 따로 잡고, 못 잡으면 못 잡았다고 적는다", "🌡️", "운영"),
    ("ops_console.html", "교배 배정 · 환경 알람", "근친 한도 아래 전체 최적 배정(헝가리안) · 지침 위반과 자기 기준선 편차를 겹으로 본 돈사 알람", "🧬", "운영"),
    ("season_interval.html", "여름 손실 · 간격 what-if", "67농장 계절 손실 분포를 우리 규모로 · 무너지는 건 착상 · 같은 돈사로 간격만 바꾸면", "☀️", "분석"),
    ...
    ("alert_console.html", "실시간 경보 콘솔", "긴급도순 조치 큐 + 모바일 푸시 — 오늘 할 일로 변환", "🔔", "분석"),
    ("repro_dashboard.html", "번식 진단·조기경보", "문제 유형 진단 + 원인 4종 귀인 + 발정 예상일(D-day) 조기경보", "🩺", "분석"),
    ("eval_report.html", "평가 신뢰도 리포트", "혼동행렬·ROC·PR·보정곡선 — 개체 분리 검증(심사 신뢰도)", "📐", "분석"),
    ("posture_report.html", "자세 인식 병목 리포트", "누수 수치 폐기 · 원리적 상한 · 기준선 대비 · 하류 전파까지", "🧍", "분석"),
...
KPIS = [
    ("발정 → 전체", "관리 범위", "발정·임신·분만·돈군흐름·생산비까지"),
    ("0.739", "활동/휴식 ROC-AUC", "개체 분리 GroupKFold — 발정 관찰의 핵심"),
    ("0.636", "자세 3클래스(교차-뷰)", "못 본 카메라 LOVO · 0.513 → 0.636"),
    ("45,611", "돼지 탐지(bbox)", "직접 학습 mAP50 0.659"),
]
...
<div class="hero">
  <h1>🐖 양돈 AI 통합 대시보드</h1>
  <div class="sub">돈방 CCTV로 <b>돼지 탐지 → 행동 인식 → 발정 판정</b>, 거기서 <b>번식 일정·돈군흐름·생산비</b>까지.
  발정은 시작점이지 끝이 아니다 — 놓친 발정은 배치를 흩뜨리고, 흩어진 배치는 돈방과 출하까지 밀어낸다.</div>
</div>
<div class="kpis">{kpi_html}</div>
<div class="flow">파이프라인: <b>탐지</b>(YOLO) → <b>추적/활동</b>(점유·활동량) → <b>행동 인식</b>(기하+모션+외형) → <b>발정 연계</b>(71471 표준) → <b>번식 일정</b>(교배·임신진단·분만) → <b>돈군흐름</b>(배치·돈방·출하) → <b>손익</b>(지렛대 순서)</div>
