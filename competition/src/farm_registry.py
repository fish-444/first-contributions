...
# --------------------------------------------------------------------------
# 번식주기 상수 — **여기서 새로 적지 않는다.**
#
# 예전에 `W2S, GEST, LACT = 7, 114, 24` 를 직접 박아 뒀는데, 같은 시각 batch_flow
# 는 임신 115 · 이유~교배 5.0 으로 돌고 있었다. 두 모듈이 같은 농장에 대해
# 다른 주기(145일 vs 144일)를 쓴 것이고, 연속 흐름 돈사의 지지 두수가 주기에
# 정비례하므로 **같은 돈사가 모듈에 따라 다른 규모로 나왔다.**
#
# 더 나쁜 건 방향이었다. 이유~교배 실측 중앙은 6.9일인데 batch_flow 가 지침
# 목표 5.0 을 쓰고 있어서, 교배사가 실제보다 **4.3% 크게** 계산됐다. 낙관 쪽으로
# 어긋난 것이라 "자리가 모자란다" 는 사실을 그만큼 늦게 알려 준다.
#
# 이제 둘 다 `batch_flow.herd_cycle()` 에서 받는다. 지침값(일정 예측용)과
# 실측값(점유·용량용)이 다른 수라는 것도 거기 적혀 있다.
def _cycle_consts() -> tuple:
    try:
        import batch_flow as _bf

        c = _bf.herd_cycle()
        return (round(c["wean_to_service"]), round(c["gestation"]),
                round(c["lactation"]))
    except Exception:                      # noqa: BLE001 — 통계 없으면 지침
        return 7, 115, 25


W2S, GEST, LACT = _cycle_consts()  # 이유~교배 · 임신 · 포유 (실측 중앙 반올림)
...
def stage_counts(n_sows: int) -> dict:
    ...
    맞지 않았다. 실측 중앙 주기 기준으로 교배사 23.9% · 임신사 54.4% ·
    분만사 21.8%(본군 대비).
    """
    n = max(1, int(n_sows))
    cycle = W2S + GEST + LACT
