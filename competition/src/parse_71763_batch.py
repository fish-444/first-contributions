...
## 실전 1회 (2026-08-25, 로컬 · 71763_PARSE.md)

라벨 JSON **360,024개**(TL 319,999 + VL 40,025) → 프레임 360,024행 ·
클립 **3,944행**. **파싱 실패·누락 0건** — 프레임 행수가 입력 파일 수와
정확히 같다. 워커 12·샤드 48 로 19분. 61% 지점에서 한 번 죽은 뒤 기존
샤드를 재사용한 재실행이 **38초**에 같은 결과를 냈다 — 체크포인트가
설계대로 동작한 증거다.

⚠️ 이 클립 3,944 는 문서(AIHUB_71763.md 0절)의 3,793 과 다르다. 두 수를
합치지 말고 어느 파일 집합에서 나온 것인지 밝혀 쓸 것 — 0-B 참조.
